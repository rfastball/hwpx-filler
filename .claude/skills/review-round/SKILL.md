...
**멈춰야 하는 신호가 둘 있다.** 게이트는 이 둘을 막지 않으므로 — 판정이 `READY` 여도 —
여기 적혀 있지 않으면 그냥 지나간다.

- **회귀 후보가 하나라도 표시되면** 그 자리에서 점별 픽스를 멈춘다(정책 §3, 첫 재발부터다).
- **차단이 3라운드 연속이면** 지적이 매번 달라도 멈춘다. 판독기가 그 수를 함께 적는다.
  같은 자리가 아니어도 같은 **결함류**일 수 있고, 그때는 하나씩 고치는 것이 답이 아니다.

멈춘 뒤에는 그 류를 구조로 닫는다 — 파생 판정 단일화·가드 계약·타입 봉합처럼, 다음 라운드에
같은 가족이 나오지 못하게 만드는 조치다.
