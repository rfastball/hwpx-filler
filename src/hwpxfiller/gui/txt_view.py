...
class TxtDraftView(QWidget):
    ...
    def refresh(self) -> None:
        """템플릿 목록 재적재 — 셸 재진입 스테일 방지(SHELL_DESIGN D6).

        은닉 중 템플릿 루트에 파일이 생기거나 지워져도 재진입 시 최신 목록을 보인다.
        현재 선택은 이름으로 보존(데이터·렌더 상태 무접촉), 사라졌으면 첫 항목 폴백.
        """
        current = self.cbo.currentText()
        names = self.vm.template_names()
        self.cbo.blockSignals(True)
        try:
            self.cbo.clear()
            self.cbo.addItems(names)
        finally:
            self.cbo.blockSignals(False)
        if current and current in names:
            self.cbo.setCurrentText(current)
        elif names:
