...
class ReviewRequirement:
    ...
    evidence_policy: str = ""
    #: 한 번도 완주한 적 없는 작업인가 — §13-3 「새 문서 작업」. **고지 대상은 아니다**
    #: (사용자 판정, 간소화 라운드 — 결과 확인은 상수라 알릴 정보가 없다). 그래도 사실은
    #: 남는다: 이 값이 :attr:`unknown_baseline` 과 갈리는 축이고, 참일 때 일반 문안
    #: ("마지막 실행 이후 바뀐 규칙")이 거짓이 되는 것을 막는다.
...
def review_requirement(
    ...
    라고 말하면 그건 거짓말이고, 거짓 문안은 경보를 싸구려로 만든다. 두 갈래 중 앞의 것은
    이제 **말하지 않는 쪽**으로 갈린다(:func:`review_notice_text`) — 구분이 필요한 이유는
    그대로다: 갈래가 없으면 첫 실행에 "마지막 실행 이후"를 말하게 된다.
...
def review_notice_text(req: ReviewRequirement) -> str:
    ...
    ``first_run`` 도 **빈 문자열**이다(사용자 판정, 간소화 라운드): 결과 문서를 열어 보는
    것은 첫 실행이든 백 번째든 **언제나 하는 일**이라, "첫 실행입니다"는 사용자가 이미
    아는 사실을 되풀이할 뿐 행동을 바꾸지 않는다. 상수인 사실은 고지가 아니다 — 매 새
    작업마다 서는 알림 줄은 알림을 싸구려로 만든다. 갈래를 지우지 않고 **빈 문자열로**
