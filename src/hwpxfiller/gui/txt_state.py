...
class TxtDraftViewModel:
    ...
    def load_saved_template(self, path: "str | Path") -> None:
        """저장 기안(Job)의 ``template_path`` 를 싣는다 — **라이브러리 이름 보존**(#148 슬라이스 5a).

        붙여넣기(:meth:`set_template_text`, 이름 없음)와 달리 저장 기안 복원은 템플릿이
        어느 라이브러리 항목인지 이름으로 말해야 한다(카드 제목·원문바 srcName). 파일 I/O 는
        이 VM 이 단일 소유(다른 층이 raw read 하지 않게) — 파일 부재는 조용히 삼키지 않고
        시끄럽게 올린다(``read_text`` 의 ``OSError``, confirm-or-alarm). 이름은 레지스트리
        루트 상대경로(list_templates 와 같은 규칙)로, 루트 밖이면 stem 으로 낙착한다."""
        p = Path(path)
        self.template_text = p.read_text(encoding="utf-8")
        try:
            self.template_name = p.relative_to(self.registry.directory).with_suffix("").as_posix()
        except ValueError:
            self.template_name = p.stem
