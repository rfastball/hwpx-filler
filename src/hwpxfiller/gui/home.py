...
레이어링(아키텍처 분리): 이 위젯은 **얇은 렌더러**다 — 목록 성형·레지스트리 접근·선택
상태는 :class:`~hwpxfiller.gui.home_state.HomeViewModel`(Qt 비의존, 링1)이 소유하고, 여기선
``vm.rows()`` 를 QListWidget 에 그린다. **네비게이션 시그널 계약은 불변** — 아이템 text 는
계속 작업 이름을 담고(findItems 선택 보존·스모크 계약), 카드는 그 위에 setItemWidget 으로 얹는다.
"""
...
from .home_state import HomeViewModel, JobRow
...
class _JobCard(QWidget):
    ...
    성형된 :class:`JobRow` 만 받는다(Job 을 직접 만지지 않음). 배경은 투명(리스트 선택
    하이라이트가 비치도록) — 이름 텍스트는 카드가 아니라 아이템이 소유하되 투명 전경으로 감춘다.
    """

    def __init__(self, row: JobRow, parent=None):
...
class JobListHome(QMainWindow):
    """작업 목록 홈. :class:`HomeViewModel` 을 렌더하고, 액션을 시그널로 방출한다.
    ...
    def __init__(self, registry: JobRegistry, parent=None):
        ...
        self.vm = HomeViewModel(registry)
    ...
    def refresh(self) -> None:
        """뷰모델에 재적재를 요청한다(app.py 배선이 저장·삭제·집행 후 호출) → _render 통지."""
        self.vm.refresh()

    def _render(self) -> None:
        """뷰모델 상태를 QListWidget/빈 상태/버튼에 반영(선택 보존)."""
        prev = self.vm.selected_name
        self.list.blockSignals(True)  # 재구성 중 selectionChanged 재진입 방지
        self.list.clear()
        ...
        # blockSignals 로 selectionChanged 가 안 떴으니 VM·버튼을 수동 동기화.
        self.vm.select(self._current_item_text())
        self._sync_buttons()

    def selected_job_name(self) -> "str | None":
        return self.vm.selected_name

    # ------------------------------------------------------------- 내부
    def _current_item_text(self) -> "str | None":
