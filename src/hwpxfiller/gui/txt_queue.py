...
작업점)를 지나간다** — 편입 순서 보존 + 처리분 후미 이동(멱등 재복사·완주=미처리 소진) +
...
- **미루기 사망**(R-info 3부 결정 10 — R-flow 결정 19 회수, #148 슬라이스 3c). 막힌 카드의
  ...
  큐 뒤로 보내는 동사가 필요 없다. 「미루기 순서 명시 보존」 계약은 소멸했고, ``reconcile``
  이 유지하는 미처리 순서는 이제 **선택 편입 순서**(선언 순서) 하나만 담보한다.
...
class TxtQueueModel:
    ...
    def __init__(self, selection: SelectionModel) -> None:
        ...
        self._uncopied_order: "list[int]" = []      # 미처리 큐(순서=선택 편입순)
    ...
    def reconcile(self) -> None:
        ...
        기존 미처리 순서는 보존한다: 선택 변경은 지형만 바꾸고 순서를 갈아엎지 않는다.
        미루기 사망(결정 10) 뒤 이 보존이 담보하는 것은 **선택 편입 순서**(선언 순서)
        하나다 — 카드를 뒤로 보내는 동사가 없어졌으므로 후미 이동은 오직 복사로만 일어난다.
        복사 상태는 선택에 종속(``copied ⊆ selected``)이라 해제된 행은 복사 이력까지 빠진다
        (재선택 시 새 미처리).
