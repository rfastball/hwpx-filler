...
- **전열 검색 = 재현 OR 그룹**(결정 23): 검색어는 "실제 매치가 있는 열"에만 가지를 세운
  ...
  가지 후보에서 일자·금액 열은 제외(범위 문법 소관 — 시안 동형).
- **자모 부분일치**(결정 23): 열 텍스트·전열 검색 모두 :mod:`~hwpxfiller.core.jamo` 소비
  (「행복도ㅅ」 단계 매치). 하이라이트는 :meth:`FilterModel.segments` 가 **파이썬에서 원문을
  잘라 세그먼트로** 준다 — 매치 인덱스를 웹으로 건네지 않는다(jamo PR-1 리뷰 계약:
  코드포인트/UTF-16 파생경계 번역오류의 상류 차단).
- **범위 조건 = 엑셀 사용자 지정 자동 필터 동형**(2026-07-19 확정): 비교 연산자 6종
  (=·≠·>·≥·<·≤) + 최대 2절 AND/OR 결합. **동적 날짜 프리셋(오늘·지난주 등)은 제외**
  (벽시계 상대 정의는 재현 재진술과 마찰 — 같은 정의가 날마다 다른 행을 매치). 피연산자
  해석은 표시형과 같은 파서(:func:`~hwpxfiller.core.format_engine.parse_number`/``parse_dt``)
  — 표시가 읽는 대로 비교한다(값 해석 단일 출처). 셀이 파싱 불가면 불매치(엑셀 동형),
  **피연산자가 파싱 불가면 설정 시점에 시끄럽게 거절**(엑셀은 조용히 문자열 비교로
  강등하지만 그건 조용한 추측이다 — confirm-or-alarm 의도적 편차).
...
- **프루닝 지속성 = 텍스트 수명**: 쳐낸 가지는 검색 텍스트를 고칠 때(=그룹 재정의)만
  복귀한다. 가지 집합 자체는 매 평가 라이브 산출이라(저장 안 함) 열-조건 편집으로 매치
...
문안(:meth:`FilterModel.describe`)은 칩 줄·게이트 재진술·「전체 선택」 담보가 공유하는
**단일 출처**다(결정 4). 층화 표본(결정 5)은 :meth:`FilterModel.stratified_sample`.
...
# 날짜 스니핑용 — format_engine.parse_dt 와 같은 골격이되, 매치 양옆이 숫자가 아닐 것을
# 추가로 요구한다(앵커 검사). parse_dt 는 관대해서 「20260715623-00」(공고번호류 연속
# 숫자런)에서도 앞 8자리를 날짜로 읽는다 — 스니핑이 그걸 날짜 열로 오판하면 텍스트
# 열에 범위 문법이 붙는다. 렌더(degrade 안전)와 달리 유형 판정은 보수적이어야 한다.
_DATEISH_RE = re.compile(r"\d{4}\D?\d{1,2}\D?\d{1,2}")
...
def _is_dateish(value: str) -> bool:
    """스니핑용 날짜 판정 — 파싱 성공 + 매치가 더 긴 숫자런에 묻혀 있지 않을 것."""
    m = _DATEISH_RE.search(value)
    if m is None or parse_dt(value) is None:
        return False
...
def sniff_column_kinds(
    ...
    확정한 유형이므로 스니핑보다 우선한다. 힌트 없는 열은 비어 있지 않은 값 전부가
    수(→amount) 또는 날짜(→date)로 읽힐 때만 승격하고, 하나라도 어긋나면 text 로
    남는다(오판은 텍스트 쪽으로 안전 강등 — 값 체크리스트·자모 검색은 전 유형 공통).
    전부 빈 열도 text. 열 순서는 첫 레코드의 키 순서(데이터 소스 열 순서 보존).
    """
    hints = hints or {}
    ...
    for col in columns:
        ...
        if values and all(parse_number(v) is not None for v in values):
...
class _ColumnCondition:
    ...
    def __init__(self) -> None:
        self.values: "set[str] | None" = None  # None=무조건, set=체크된 값만(OR)
        self.text = ""  # 자모 부분일치
...
class FilterModel:
    ...
    레코드를 소유하지 않는다 — 평가 메서드가 매번 ``records`` 를 받는다(데이터 소유는
    링1 VM, 이 모델은 술어만). 가지 집합은 저장하지 않고 매 평가 산출한다(stale 없음),
    프루닝만 텍스트 수명으로 기억한다(미결 확정 1).
    """
    ...
    # ------------------------------------------------------------- 변경(선언)
    def set_values(self, column: str, values: "set[str] | None") -> None:
        """값 체크리스트(같은 열 OR) — ``None``=무조건, 집합=체크된 값만. ``""``=(빈값) 일급."""
        self._require(column)
        self._cols[column].values = None if values is None else set(values)

    def set_text(self, column: str, text: str) -> None:
        """열 텍스트 조건(자모 부분일치) — 빈 문자열이면 조건 해제."""
        self._require(column)
        self._cols[column].text = text

    def set_range(self, column: str, cond: "RangeCondition | None") -> None:
        """범위 조건 — 일자·금액 열 전용, 피연산자는 설정 시점에 파싱 검증(시끄러운 거절).

        엑셀은 파싱 불가 피연산자를 조용히 문자열 비교로 강등하지만 그건 조용한 추측이다
        — 여기서는 설정 자체를 거절해 정의가 항상 해석 가능함을 담보한다(의도적 편차).
        """
        self._require(column)
        if cond is None:
        ...
        for clause in (cond.first, cond.second):
            ...
            if self._parse_operand(kind, clause.operand) is None:
                noun = "금액" if kind == KIND_AMOUNT else "날짜"
                raise ValueError(
    ...
    def prune_branch(self, column: str) -> None:
    ...
    def group_branches(self, records: "list[dict]") -> "list[str]":
        """전열 그룹의 가지 — 실매치 있는 텍스트 열만, 프루닝 반영. 매 평가 라이브 산출.

        가지 설치 판정은 열 조건을 통과한 행 기준(시안 ``recomputeBranches`` 동형) —
        조건이 이미 배제한 행에서만 맞는 열에 가지를 세우면 재현이 거짓말이 된다.
        """
        if not self._search:
            return []
        return [
            ...
            and col not in self._pruned
            and any(
                self.col_pass(r) and jamo_contains(_cell(r, col), self._search)
                for r in records
            )
        ]

    def _group_pass(self, record: "dict", branches: "list[str]") -> bool:
        if not self._search:
            return True
        if not branches:  # 어느 열에도 매치 없음 = 전멸(정직 — 빈 화면이 재진술)
            return False
        return any(jamo_contains(_cell(record, b), self._search) for b in branches)

    def visible_indices(self, records: "list[dict]") -> "list[int]":
        """필터를 통과한 행 인덱스(원본 순서) — 보기만 바꾼다, 선택은 관통(결정 3)."""
        branches = self.group_branches(records)
        return [
            i for i, r in enumerate(records)
            if self.col_pass(r) and self._group_pass(r, branches)
        ]
    ...
    def column_values(self, column: str, records: "list[dict]") -> "list[str]":
        ...
        has_empty = False
        for r in records:
            if not (self.col_pass(r, except_column=column) and self._group_pass(r, branches)):
                continue
            v = _cell(r, column)
            if v == "":
                has_empty = True
    ...
    def _describe_column(self, column: str) -> "list[str]":
        ...
        if cond.values is not None:
            vals = sorted(cond.values, key=lambda v: (v == "", v))  # (빈값) 말미 안정 정렬
            if len(vals) == 1:
    ...
    def describe_parts(self, records: "list[dict]") -> "list[str]":
        ...
        if self._search:
            branches = self.group_branches(records)
            if branches:
                parts.append(f"({' ∨ '.join(branches)}) 포함 「{self._search}」")
            else:
                parts.append(f"검색 「{self._search}」 — 매치 없음")
        return parts

    def describe(self, records: "list[dict]") -> str:
        """정의줄 전체 — 「전체 선택」·게이트 재진술이 그대로 나른다(문안 단일 출처)."""
        return " · ".join(self.describe_parts(records))
    ...
    def stratified_sample(
        ...
        if limit <= 0:
            return []
        branches = self.group_branches(records)
        if not self._search or not branches:
        ...
        for branch in branches:
            for i in indices:
                if i in picked:
                    continue
                if jamo_contains(_cell(records[i], branch), self._search):
                    picked.append(i)
                    break
        for i in indices:
    ...
    def segments(
        ...
        상류 차단). 적용 순서 = 열 텍스트 조건 → 전열 검색(그 열이 가지일 때) — 첫 매치
        하나만 칠한다(시안 ``mark`` 동형). 매치 없으면 통짜 한 조각.
        """
        self._require(column)
        terms: "list[str]" = []
        if self._cols[column].text:
            terms.append(self._cols[column].text)
        if self._search and column in self.group_branches(records):
            terms.append(self._search)
        for term in terms:
