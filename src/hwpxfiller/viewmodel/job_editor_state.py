...
#
# 설명 줄(종전 ``BINDING_CONFIRM_HINT``)은 걷혔다 — 전제 조건 낭독은 문안이 아니라 동사가
# 진다(`docs/ui-style.md#ui-copy` 낭독 패턴 1). 무장된 버튼 하나가 라벨로 말하고, 왜
# 무장됐는지는 관리 검토의 blocker 가 이미 사유를 든다.
BINDING_CONFIRM_LABEL = "연결 확정"

# ── 작업 이름 기본값(U6-D #978) ─────────────────────────────────────────────────────────
#: 두 이름을 잇는 **이름 구분자**. em dash 가 아니라 가운뎃점이다(U6 §5 미결 항목의 확정):
#: `docs/ui-style.md#ui-copy` 이 문장 안 em dash 를 전면 금지하는데, 라벨 문형과
