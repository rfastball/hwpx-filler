...
run 과 동일 경계. 풀 겨눔은 링1 :meth:`~hwpxfiller.gui.matrix_state.MatrixRunViewModel.
load_pool_item` 재사용. **나라장터 소스는 동결**(#10/#24 정합) — 풀의 nara 항목은 목록에
보이되 겨눔은 :mod:`.screens` ``load_pool_item_checked`` 단일 관문이 시끄럽게 거절한다.

**이번 이관의 스코프 경계(조용히 빠뜨리지 않고 명시)** — 아래는 미구현이며 후속
...
class MatrixController:
    ...
    def __init__(
        ...
        *,
        ...
        # 등록 데이터(풀) 겨눔(#26/#6) — VM 에 풀 레지스트리를 주입(기본=홈 레지스트리).
        # 나라 항목은 load_pool_item_checked 관문이 동결 거절한다(웹 2소스 경계).
        self.pool_registry = (
