...
class TxtController:
    ...
    def dispatch(self, action: str, payload: dict) -> None:
        ...
        if action == "select_template":
            self.vm.select_template(payload["name"])
        elif action == "set_template_text":
            self.vm.set_template_text(payload["text"])
        elif action == "step":
            self.vm.step(int(payload["delta"]))
        else:  # confirm-or-alarm: 조용한 무시 금지.
