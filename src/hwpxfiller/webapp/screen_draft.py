...
class DraftController(DraftSessionMixin):
    ...
    def _do_save_job(self, p: dict) -> "dict | None":
        ...
        **저장 = 확정**: 휘발 세션은 확정 열이 숨어 있어 행이 미확정이므로, 승격이 내용 있는 행을
        ...
        # 덮어쓰기 확인 — 다른 기존 기안을 덮게 될 때만(자기 재저장은 자명). 파괴 대상을 재진술.
        if name != self._bound_job and self.registry.exists(name) and not p.get("confirm"):
            try:
        ...
        if not self._bound_job:  # 휘발 승격 — 확정 열이 숨어 있었으니 저장이 확정한다(내용 있는 행)
            for i, r in enumerate(self.mapping.rows):
                if r.has_content():
                    self.mapping.set_confirmed(i, True)
        group = ""
        if self.registry.exists(name):  # 재저장/덮어쓰기 = 기존 그룹 보존(조용한 그룹 소거 금지)
            try:
                group = self.registry.load(name).group
            except (FileNotFoundError, ValueError):
                group = ""
        job = Job(name=name, template_path=self._template_path,
                  mapping=self.mapping.to_profile(name), group=group)
        with self.registry.write_lock():  # 생성 스레드 last_run 스탬프와의 lost-update 방지
