...
**슬라이스 1 스코프 경계**(confirm-or-alarm: 없는 기능을 있는 척하지 않는다) — 아래는 후속 슬라이스:
- 게이트 재진술 블록·거울 배지 UI 광택·덮어쓰기 modal.js(블록 6 = 슬라이스 2). 슬라이스 1은
  링1 계약(``output_conflicts``·ack 상태기계)을 배선하되 표현은 소박하게 렌더한다.
- 식별 요약 링1 함수(#88, A-1-15) — 슬라이스 1은 임시 요약(:func:`_record_summary`)을 쓰고
  #88 착지 시 대체한다.
- 좌 목록의 2구획 틴트·group-by 렌즈·컴파일 배지 등 풍부화(홈 브라우저 VM 채택은 후속).
- 필터·세션 가드·건 연속성(블록 4 = 슬라이스 4)·txt 큐(블록 3)·빠른 기안(블록 5).
- 미구현 스코프 3종(나라 겨눔·협조적 취소 RC-06·생성 원장 opt-in)은 #89 가 docstring 경계로 이관.
"""
...
# 임시 식별 요약(F33) — 원본 레코드 앞쪽 값 N열. 결정 37 식별 요약 휴리스틱 v2(#88, A-1-15)가
# 링1 단일 함수로 대체 예정. 슬라이스 1은 이 소박한 판을 소비한다(조용한 빈 요약 금지).
_SUMMARY_FIELDS = 3


def _record_summary(rec: dict) -> str:
    """원본 레코드 앞쪽 값들로 만드는 한 줄 식별 요약 — 빈 값은 건너뛴다(#88 대체 예정)."""
    parts = [
