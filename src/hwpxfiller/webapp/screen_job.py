...
- 세션 가드의 유보 성분: **T2**(마법사 인계 비파괴화 — 에디터 접합, 슬라이스 5)·**T3**(txt
  큐 부분 진행 성분 — 슬라이스 6)·수동 필드 입력 성분(블록 5, 슬라이스 7).
...
class JobController(PoolTargetingMixin):
    ...
    def _guard_state(self, vis_set: "set[int] | None" = None) -> dict:
        ...
        무장 해제) ∧ **정의-유래**(현 필터 매치 전체)가 아니고 ∧ **전체 선택**(1클릭 재현)도
    ...
    def load_data_path(self, path: str, *, sheet: "str | None" = None) -> None:
        ...
        self._data_key = f"file:{path}"  # 소스 일치 게이트(결정 28) — 라벨 아닌 전체 경로
    ...
    def _do_filter_reapply(self, p: dict) -> dict:
        ...
        열 결손 백스톱: 먼저 초안 모델에 설치해 보고 **전탈락이면 현 정의를 건드리지 않고
        거부**(이유 반환), 부분 탈락이면 설치 후 탈락 목록을 고지 소재로 돌려준다. 프루닝
        포함 복원(결정 27 소실 창의 복원). 재적용 뒤 편집은 보통 필터와 동일(별도 수명 없음).
        """
        fm = self._filter_or_raise()
        if not self._reapply_available():
            raise ValueError("재적용할 직전 필터가 없습니다.")  # 표면 오배선(버튼 미노출이 정상)
        state = self._last_filter["state"] if self._last_filter else {}
        probe = FilterModel(fm.columns, {c: fm.kind(c) for c in fm.columns})
    ...
    def _after_pool_load(self, records: list) -> None:
        ...
        self._data_key = f"pool:{self.data_label}"  # 라벨은 믹스인/자동 조준이 이미 세팅
        self.selection = SelectionModel(len(records))  # 데이터 변경 → 전체 선택 초기화
