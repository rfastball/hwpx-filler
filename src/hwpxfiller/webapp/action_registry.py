...
_REGISTRY: dict[str, dict[str, PayloadSchema]] = {
    ...
    "editor": {
        ...
        # 등록 데이터(풀) 겨눔(#932 U4-C S2-5) — `job` 화면의 `load_pool` 과 이름을 나눠
        # 쓰지 않는다: 이 표가 곧 화면별 경계의 정의라 같은 이름이 두 화면에 걸리면
        # 「누가 무엇을 받는가」가 이름 하나로는 안 읽힌다. 고르기 단계의 우 열이 `pool`
        # 채널을 직접 구독하게 된 뒤(U6-B #976) **조회 액션 `pool_options` 는 퇴역**했다 —
