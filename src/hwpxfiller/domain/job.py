...
class Job:
    ...
    group: str = ""
    # S3 템플릿 권위 Work identity(S3-09 #659). ""=아직 결속 없음 — 첫 [변경사항 확인]이
    # 명시적으로 발급한다(조용한 migration 아님). **이름은 identity 가 아니다**: 이름은
    # 개명·삭제 후 재사용이 가능해 이름을 키로 쓰면 재활용된 이름이 남의 권위 이력을
    # 물려받는다. 이 필드는 작업 실체를 따라가므로 개명·보존 삭제 복원에 생존하고,
