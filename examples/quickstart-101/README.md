...
2. 화면은 **두 열**이다: 왼쪽 *현재 데이터 → 생성 대상 문서 표 → 본문 확인 → 생성 결과*,
   ...
   ![세션 패널 — 자동 연결·3건 선택](img/07-session-panel.png)
   *그림 7. 기본 데이터 자동 연결 고지 + 3건 선택 + 오른쪽 후보 카드에 이 작업이 활성.*
...
3. **본문 확인**(거울)으로 내려가 문서에 들어갈 값을 훑는다. 필드마다 표본 값과
   **채움** 칩이 보이면 정상이다(빈 값이 있으면 여기 빨간 행이 뜬다 — 아래 안전장치).
   ...
   ![본문 확인 — 거울 표](img/09-mirror-check.png)
   *그림 9. 거울 표 — 이 문서에 실제로 들어갈 값과 채움 상태를 생성 전에 확인한다.*
...
4. 하단 **[이 작업으로 문서 생성]** 을 누른다. 진행바("생성 중… 1/3")가 돌고, 끝나면
   ...
   *그림 10. 생성 결과 구획 — 완료 태·요약·저장 폴더 어포던스, 그 아래 실행 기록.*
...
2. 같은 구획의 **[데이터 선택…]** → 「다른 데이터」의 **[파일 찾아보기…]** →
   ...
   **"이대로 복사됩니다"** — 보이는 그대로가 복사 결과다.
   ...
   ![기안 — 채움 미리보기](img/11-draft-filled.png)
   *그림 11. 값 자리는 음영으로 표시된다. ② 맞추기 표에서 토큰↔열 대응을 손볼 수 있다.*
...
4. **[📋 복사]** — 클립보드에 들어간다. **"복사 후 다음으로"** 를 켜 두면 복사할 때마다
   ...
   **"✓ 1행 전량 채움 복사 완료."** 처럼 상태를 남긴다.
   ...
   ![기안 — 복사 완료](img/12-draft-copied.png)
   *그림 12. 복사 뒤 — 카드 제목이 「복사됨」으로 바뀌고 「✓ 1행 전량 채움 복사 완료.」 노트가 남는다.*
...
![오류 연습 — 미치환 토큰 경고](img/13-missing-token.png)
*그림 13. 데이터에 없는 토큰은 빨간 `{{담당연락처}}` 로 남고 상태 pill 이 「항목 없음 1」이 된다.*
