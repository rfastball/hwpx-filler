...
1. 오른쪽 **[「문서 작업」에서 고르기]**(또는 위쪽 탭 **문서 작업**)로 넘어가
   화면 머리의 **[＋ 새 작업]** 을 누른다. 화면이 **편집 모드**로 바뀌고 3단계가 뜬다:
   *템플릿 선택 → 필드 매핑 → 작업 저장*.
...
2. **1단계 — 템플릿 선택**: 템플릿 라이브러리 목록에서 `발주요청서.hwpx` 행의
   **[이 템플릿으로]** 를 누른다. 아래에 **필드 6개**의 스키마 표(필드·추정 타입·위치·
   ...
   ![1단계 — 라이브러리에서 템플릿 선택](img/03-template-pick.png)
...
3. **2단계 — 필드 매핑**: 머리의 **[파일 선택…]** 으로 `data/발주목록.csv` 를 고른다.
   ...
   **[다음 ▶]**. (자동 제안은 초안이라, 맞더라도 한 번 확정해야 넘어간다 — "조용히
   ...
   ![2단계 — 매핑 확정 6/6](img/04-mapping-confirm.png)
...
4. **3단계 — 작업 저장**: **작업 이름**에 `발주요청서`, **파일명 패턴**을
   ...
   ![3단계 — 이름과 파일명 패턴](img/05-save-job.png)
   *그림 5. 파일명 패턴은 라이브 예시로 결과를 미리 보여 주고, 데이터 함께 등록이 기본으로 켜져 있다.*

**문서 작업** 탭 목록에 `발주요청서` 가 생긴다.
