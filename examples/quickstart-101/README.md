...
| 탭 | 하는 일 |
|---|---|
| **문서 만들기** | 데이터를 고르고, 그 데이터로 쓸 문서 작업을 실행하는 곳(트랙 A) |
| **문서 작업** | 저장된 작업을 찾고 상태를 확인·정리하는 곳. 「＋ 새 작업」도 여기 |
| *구분선 오른쪽* | |
| **기안** | 평문 초안(TXT)을 채워 복사하는 곳(트랙 B) |
| **템플릿 관리** | HWPX·TXT 라이브러리 — 상태 점검·누름틀 변환·그룹 정리 |

구분선 오른쪽 둘은 **정리 중인 자리**다 — 곧 앞의 두 탭 안으로 합쳐지며 사라진다(탭에
마우스를 올리면 그렇게 적혀 있다). 101 은 **문서 만들기**·**문서 작업**(트랙 A)과
**기안**(트랙 B)을 쓴다.
...
1. 위쪽 탭 **기안**(구분선 오른쪽). 오른쪽 세션 패널의 **① 데이터** 구획에서 템플릿 콤보를
   `발주요청_기안.txt` 로 고른다. 미리보기 카드에 원문이 뜬다.
   (내 초안을 바로 쓰려면 **[템플릿 붙여넣기…]** 로 텍스트를 부어 넣어도 된다.)
2. 같은 구획의 **[데이터 선택…]** → 「다른 데이터」의 **[파일 찾아보기…]** →
   `data/발주목록.csv`. **기안 대상 행** 표에 3건이 뜬다. 여기서도 채울 행은 직접
   고른다 — **[전체 선택]** 을 누르면 카드가 첫 행 값으로 채워진다. 카드 제목은
   **"이대로 복사됩니다"** — 보이는 그대로가 복사 결과다.

   ![기안 — 채움 미리보기](img/12-draft-filled.png)
   *그림 12. 값 자리는 음영으로 표시된다. ② 맞추기 표에서 토큰↔열 대응을 손볼 수 있다.*
...
4. **[📋 복사]** — 클립보드에 들어간다. **"복사 후 다음으로"** 를 켜 두면 복사할 때마다
   ...
   **"✓ 1행 전량 채움 복사 완료."** 처럼 상태를 남긴다.
   ...
   ![기안 — 복사 완료](img/13-draft-copied.png)
   *그림 13. 복사 뒤 — 카드 제목이 「복사됨」으로 바뀌고 「✓ 1행 전량 채움 복사 완료.」 노트가 남는다.*
...
![오류 연습 — 미치환 토큰 경고](img/14-missing-token.png)
*그림 14. 데이터에 없는 토큰은 빨간 `{{담당연락처}}` 로 남고 상태 pill 이 「항목 없음 1」이 된다.*
