...
이 폴더의 학습용 템플릿과 CSV 를 **단일 진실원**(이 스크립트에 박힌 필드 레이아웃과
``RECORDS``)에서 결정적으로 파생한다. 같은 입력 → 같은 출력이라, 손으로 연 hwpx 가
어쩌다 깨져도 언제든 되돌린다. ``scripts/gen_scenario_fixtures.py`` 의 축소판이다.

무엇을 만드나
--------------
- ``templates/발주요청서.hwpx`` — 6필드 전량 템플릿(101 학습용).
- ``templates/구매요청서.hwpx`` — 그중 4필드만 쓰는 부분집합 템플릿(102 조합 1).
- ``data/발주목록.csv`` (utf-8-sig) — 한글 헤더 = 템플릿 필드(직접 매칭). 3건.
- ``data/발주목록_2.csv`` (utf-8-sig) — 같은 헤더의 둘째 배치(102 조합 2). 2건.

넷을 함께 내는 이유는 102 의 두 조합(한 데이터 → 여러 템플릿 / 여러 데이터 → 한 템플릿)
이 **두 축을 각각 한 번씩** 바꿔 보이려면 템플릿도 데이터도 둘씩 있어야 하기 때문이다.
목록을 늘리거든 이 머리말도 같이 고친다 — 아래 ``TEMPLATES``·``main()`` 이 실제 산출물이다.
