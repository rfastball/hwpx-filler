...
칸 이름은 표시형 제안 축이다(#915 의 역사)
-------------------------------------------
필드의 유형은 템플릿 어디에도 적혀 있지 않고, 제품은 **이름 문자열**로 표시형 후보를
제안한다(:func:`hwpxfiller.domain.schema.infer_type`: ``기한``·``일자``·``날짜`` 등이 들면
date). 한때는 그 추정이 **값 검증 게이트**이기도 해서, 「계약 후 90일 이내」 같은 자유서식
값을 담는 칸에 ``기한`` 이 들면 관리 경로가 그 행을 「먼저 데이터 문제를 확인하세요」로
막았다 — 커리큘럼에 없는 게이트를 고급·심화 티어 사용자 전원이 만난 것이 #915 다. 그
게이트는 값 유형 어휘째 퇴역했다(소스 값은 언제나 타입 없는 텍스트, 값 정합은 사람 몫).
남아 있는 값 게이트는 결측·명시 null·빈/공백뿐이다.

이 자산의 칸 이름이 ``납품조건`` 인 것은 그 시절 조치의 흔적이고, 지금은 자유서식 값에
자유서식 이름이 붙어 있어 그대로가 옳다. 날짜 표시형을 배우는 자리는
quickstart-101(``납품기한`` = ISO 날짜)이 계속 진다. **자산 bytes 는 이 판정으로 바뀌지
않는다.**
