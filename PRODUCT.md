...
문서나르미는 반복 입력을 재사용 가능한 작업으로 묶는다. 사용자는 실제 값과 파일명을 검토한 뒤 HWPX를 생성하거나, TXT 내용을 검토·복사한다. 성공은 사용자가 자동 연결이나 빈 값을 조용히 넘기지 않고 문서 결과를 확인하여 완성하는 것이다.
