...
def strip_comments(css: str) -> str:
    """`/* … */` 를 걷어 **규칙만** 남긴다 — 선택자 존재/부재를 substring 으로 물을 때 쓴다.

    이 파일들의 주석은 결정 근거를 길게 적고 **죽은 선택자 이름을 일부러 남긴다**(U2 §2.11 이
    `.job-item` 을 "산출자 0곳이라 목록에서 내렸다"고 적어 두는 식 — 다음 사람이 되살리려 할 때
    근거를 만나게 하려고). 그 산문을 규칙으로 세면 "없어야 하는 선택자가 있다"는 거짓 실패가
    난다. 계약은 규칙에 관한 것이므로 세기 전에 산문을 걷는다.

    호출 순서 주의: 구간 컷은 주석 텍스트로 자르므로(`/* 부유 메뉴`) **자른 뒤에** 걷는다.
    """
