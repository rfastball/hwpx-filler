...
def test_n29_an_unlanded_p1_stage_is_still_caught(mutable: dict[str, Any]) -> None:
    """P1-02D 를 등록해도 **다른** P 단계는 여전히 어휘 밖이다 — 등록 단위는 자산을 든 착지다.

    이 대조가 없으면 단계 어휘 확장이 「P 접두는 다 통과」로 읽히는 다음 편집을 조용히
    받아 준다(집합 하나를 넓히고 형제 검사를 안 세우는 그 결함류). P 축이 자산 행을 들고
    착지해 그 이름이 KNOWN_STAGES 에 오르면 이 표본은 어휘 밖 이름으로 교체된다 — 제외로만
    정산되는 축은 착지해도 어휘에 오르지 않으므로 그 이름은 표본으로 계속 유효하다
