...
def test_guide_docs_quote_button_names_that_exist_in_the_app() -> None:
    ...
    **이 단언은 하한이다.** 문자열 존재만 보므로, 주석·이력에만 남은 죽은 이름은
    통과한다(그 축은 RETIRED_VOCABULARY 가 손으로 진다). 대신 allowlist 가 0이라
    라벨을 개명하면 **예외 없이** 여기서 걸린다 — 값을 손보며 목록도 같이 손보는
    길이 없다는 것이 이 게이트의 값이다.
    """
