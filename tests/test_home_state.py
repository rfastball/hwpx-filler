...
def test_rows_shape_meta_and_missing_template(tmp_path):
    vm = HomeViewModel(_reg(tmp_path), engine=make_hwpx_engine(), inspect_status=template_compile_status)
...
def test_empty_registry(tmp_path):
    vm = HomeViewModel(JobRegistry(tmp_path), engine=make_hwpx_engine(), inspect_status=template_compile_status)
...
def test_refresh_preserves_live_selection(tmp_path):
    reg = _reg(tmp_path)
    vm = HomeViewModel(reg, engine=make_hwpx_engine(), inspect_status=template_compile_status)
    vm.select("공고서")
...
def test_jobrow_from_job_direct():
    row = JobRow.from_job(Job(name="x", template_path="", filename_pattern="p-{{ID}}"), engine=make_hwpx_engine(), inspect_status=template_compile_status)
...
def test_corrupt_job_file_surfaces_as_corrupt_row_not_crash(tmp_path):
    ...
    vm = HomeViewModel(reg, engine=make_hwpx_engine(), inspect_status=template_compile_status)  # 생성자 refresh 가 JSONDecodeError 로 죽지 않는다
...
def test_only_corrupt_files_is_not_empty_state(tmp_path):
    ...
    vm = HomeViewModel(JobRegistry(jobs_dir), engine=make_hwpx_engine(), inspect_status=template_compile_status)
    assert vm.rows() == []
...
def test_dashboard_kpi_from_real_data_and_empty_defaults(tmp_path):
    ...
    vm = HomeViewModel(_reg(tmp_path), TextTemplateRegistry(td), engine=make_hwpx_engine(), inspect_status=template_compile_status)
    k = vm.kpi()
    ...
    empty = HomeViewModel(reg, engine=make_hwpx_engine(), inspect_status=template_compile_status).kpi()  # txt 레지스트리 없음
...
def _row(template_path: str) -> JobRow:
    return JobRow.from_job(Job(name="작업", template_path=template_path), engine=make_hwpx_engine(), inspect_status=template_compile_status)
...
def test_axes_discovered_from_tags_union(tmp_path):
    ...
    vm = HomeViewModel(_tagged_reg(tmp_path), engine=make_hwpx_engine(), inspect_status=template_compile_status)
...
def test_untagged_corpus_is_degenerate_flat(tmp_path):
    ...
    vm = HomeViewModel(_reg(tmp_path), engine=make_hwpx_engine(), inspect_status=template_compile_status)  # 태그도 그룹도 없는 기존 픽스처
    assert vm.axes() == []
    assert vm.facets() == []
...
def test_library_views_project_without_new_state(tmp_path):
    ...
    vm = HomeViewModel(_library_reg(tmp_path), engine=make_hwpx_engine(), inspect_status=template_compile_status)
...
def test_library_all_view_sections_by_group_and_degenerates(tmp_path):
    ...
    flat = HomeViewModel(plain, engine=make_hwpx_engine(), inspect_status=template_compile_status)
...
def test_library_ungrouped_section_is_identified_apart_from_flat_degenerate(tmp_path):
    ...
    plain.save(Job(name="가", template_path=""))
    flat = HomeViewModel(plain, engine=make_hwpx_engine(), inspect_status=template_compile_status).library_sections()
...
def test_health_causes_do_not_misdiagnose_unlinked_as_unsupported(tmp_path):
    ...
    row = next(r for r in HomeViewModel(reg, engine=make_hwpx_engine(), inspect_status=template_compile_status).rows() if r.name == "저작중")
