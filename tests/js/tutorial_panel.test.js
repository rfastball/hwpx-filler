...
/* 회피 규칙(#918-B)의 CSS 소비자 대조와 실렌더 기하 게이트는 여기 있었다. 튜토리얼 진입
   표면이 배포본에서 걷히며(#941) `.tut-*` 규칙과 `tests/test_web_tutorial_geometry.py` 가
   함께 사라졌으므로, 이 파일이 지는 것은 **패널 컴포넌트의 렌더 계약** 하나다 — 표면을
   되살리는 변경이 그 CSS 와 기하 층을 함께 되돌린다. */
