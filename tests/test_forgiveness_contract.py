...
def test_confirm_inventory_is_net_lower_than_audit_ledger() -> None:
    ...
    #
    # +2(재작성 F6): 작업대의 ①복사 전 결손 확인 ②이탈 시 복사 진행 소실 확인. 둘 다
    # **조건부**다 — 결손(미치환·빈 값)이 있을 때만, 가드가 무장했을 때만 뜬다. 그리고 둘
    # 다 「기안」 세션이 이미 갖고 있던 확인의 작업대 판이라, PR-B 에서 그 화면이 죽으면
    # draftsession.js 의 같은 확인이 걷혀 **원장은 다시 내려간다**(순증이 아니라 이사다).
    count = sum(
