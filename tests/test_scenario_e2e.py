...
번들 구성(같은 조달 어휘 23필드 공유 — 개찰일시가 개찰일자+개찰시각으로 분해됨)::

    templates/입찰공고서.hwpx       23필드 전량(scenario 전용 저작 — real 재사용 아님)
...
- **직접 매칭 경로**: 한글 CSV → Job/RunRequest/generate_batch → 22필드 채워진 산출.
...
BID_NOTICE = str(TEMPLATES / "입찰공고서.hwpx")   # 23필드(개찰일자/개찰시각 분리)
...
def test_same_data_fills_subset_template_ignoring_extra_keys(tmp_path):
    ...
    assert "계약방법" in res0.unmatched  # 22필드 CSV엔 있으나 10필드 템플릿엔 없음
...
def test_txt_template_from_korean_csv_record():
    ...
    assert report.missing_fields == []       # 한글 CSV 는 22필드 전량 제공
