...
scenario/
├─ templates/
│   ├─ 입찰공고서.hwpx      23필드 전량 — scenario 전용 저작(개찰일자/개찰시각 분리)
...
모든 템플릿·데이터는 **같은 조달 어휘 23필드**를 공유한다. 그래서 한 소스가 여러
템플릿을 채우고, 소스 어휘 차이(영문 vs 한글)는 매핑 프로파일이 흡수한다.

**일시 단일프레임 분해.** 엄격 1:1 매핑 모델(한 필드 ← 한 소스 키, 결합 없음)에 맞춰
`개찰일시`를 `개찰일자`(date 기본 서식) + `개찰시각`(date `%H:%M`) 두 필드로 나눴다.
CSV/xlsx 는 두 컬럼으로, txt 는 인접 `{{개찰일자}} {{개찰시각}}` 토큰으로, hwpx 는 두
누름틀로 심는다. 나라장터 매핑은 `opengDate`/`opengTm` 을 각각 잇는다.

## 매칭 표

| 소스 | 어휘 | 매핑 필요 | 입찰공고서(23) | 구매요청서(10) | txt 기안 |
|------|------|:---:|:---:|:---:|:---:|
| `조달_한글.csv` / `.xlsx` | 한글 헤더 = 필드 | ✗ (직접) | 전량 채움 | 부분집합 채움·여분 무시 | 원문 값 |
| `나라장터_응답.json` | 영문 코드 키 | ✓ `나라장터_매핑.json` | 14필드 채움·9필드 미충족 | — | 서식 값(WYSIWYG) |

핵심 대비: 한글 CSV 는 세부품명·수량 등 23필드를 **직접** 채우지만, 나라장터 소스엔
그 값이 원천에 없어 해당 누름틀이 **미충족으로 남는다**(더 풍부한 CSV 를 선호할 실증
근거). 반대로 나라장터 경로는 매핑이 금액(`45,000,000원`)·일시(개찰일자 `2026년 7월 22일`
+ 개찰시각 `10:00`)를 정규화한다.
