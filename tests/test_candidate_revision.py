...
@pytest.mark.parametrize(
    ...
    (
        ...
        (MutableSourceBinding("SB1", "hwpx", "u", {}, 8), LINEAGE, None, SOURCE_BINDING_CHANGED),  # gen 7≠8
    ),
)
...
def test_file_reader_detects_same_size_overwrite_that_metadata_misses(tmp_path) -> None:
    # 같은 길이·mtime 보존 in-place 덮어쓰기 — ino/dev/size/mtime 은 그대로라 metadata 판정은
    # 통과하고, 오직 두 번째 read 의 내용 대조만 불안정을 잡는다.
    src = tmp_path / "template.hwpx"
    src.write_bytes(BYTES)
    binding = MutableSourceBinding("SB1", "hwpx", str(src), {}, 7)
    orig = src.stat()

    def preserve_mtime_overwrite() -> None:
        src.write_bytes(b"Y" * len(BYTES))
        os.utime(src, ns=(orig.st_atime_ns, orig.st_mtime_ns))

    reader = FileTemplateSourceReader(lambda _sid: 7, interference=preserve_mtime_overwrite)
    assert reader(binding) == SourceCaptureError(SOURCE_CAPTURE_ERROR)


def test_get_blob_rejects_mismatched_algorithm(tmp_path) -> None:
    store = CandidateObjectStore(tmp_path)
    store.put_blob(ContentBlob(blob_digest(BYTES), "hwpx", BYTES, len(BYTES)))
    forged = f"sha512:{blob_digest(BYTES).split(':')[1]}"  # 같은 hex, 다른 알고리즘
    assert not store.has_blob(forged)
    with pytest.raises(ObjectNotFound):
        store.get_blob(forged)


def test_put_revision_requires_observation_to_prove_the_blob(tmp_path) -> None:
    from hwpxfiller.application.candidate_revision import (
    ...
    store.put_observation(
        SourceCaptureObservation("O1", "P1", "SB1", 7, "m", {}, blob_digest(b"other bytes"), "t1")
    )
    rev = TemplateRevision("R1", "L1", "hwpx", blob_digest(BYTES), "O1", "t1")
    with pytest.raises(DanglingReference):  # observation 은 다른 bytes 를 capture 했다
        store.put_revision(rev)


def test_observation_metadata_snapshots_reader_dict(tmp_path) -> None:
    store = CandidateObjectStore(tmp_path)
    meta = {"byte_length": len(BYTES)}

    def reader(_binding):
        return StableCapture(BYTES, blob_digest(BYTES), "SB1", 7, "fake", meta)

    _capture(store, reader=reader)
    meta["byte_length"] = 999  # capture 이후 reader-owned dict 변이
    assert store.get_observation("O1").observed_metadata["byte_length"] == len(BYTES)
