...
앱을 띄우면 「문서 만들기」 화면이 열립니다. 첫 문서까지 세 걸음입니다.

1. 위쪽 탭 **문서 작업** → **[＋ 새 작업]** — 라이브러리에서 누름틀 템플릿을 고르고,
   엑셀/CSV 를 골라 필드를 매핑한 뒤, 작업 이름과 파일명 패턴을 붙여 저장합니다.
2. **문서 작업**에서 그 작업을 고르고 **[문서 만들기에서 사용]** — 만들 행을 고른 뒤
   **[이 작업으로 문서 생성]** 이면 저장 폴더에 완성 HWPX 가 선택한 건수만큼 생깁니다.
3. 다음부터는 **문서 만들기**에서 데이터를 고르면 그 데이터로 쓸 수 있는 작업이
   후보로 뜹니다 — 골라서 생성하면 됩니다.
