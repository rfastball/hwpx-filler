...
**문서 상태: 역사 기록 — 완주·동결(2026-07-29)**. v6 표면 전면 재작성의 전 슬라이스
(R1·F1~F8)가 착지해 이 지도가 소유하던 대조·슬라이싱·승계 의무는 **전부 정산됐다**(§10.17.6).
**새 판정을 여기에 덧붙이지 않는다** — 착지 이후 라운드의 정본은 별도 문서가 진다
([U2 실사용 피드백 라운드](../UX_FEEDBACK_U2.md)). 이 문서는 **왜 그런 표면이 됐는지**의
근거 원장으로 동결되며, 절 번호(§7·§8.2·§10.x 판정)는 계속 인용 대상이다.

원래 정체는 v6 UI 랩의 `docs/core-workflow-integration-plan.md`(봉합 계획) 단계 1 산출물이다.
워크플로 계약 정본은 [`docs/core-workflow.md`](../core-workflow.md)이고, 이 문서는 그 계약과
master 실물 사이의 **소유권 대조표**만 소유한다. 인용의 `lab:`은 동결 시점 랩 워크트리,
무접두는 이 브랜치(master 계열) 기준이다.
