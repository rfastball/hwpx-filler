# UI 계약 — 목업 ↔ ViewModel seam (앱 B, Qt 시대 보존본)

> **문서 상태:** 역사 기록
> **권위 범위:** Qt 셸 시대 목업 ↔ ViewModel seam과 당시 디자인 워크플로
> **후속 정본:** [현재 웹 UI 계약](../UI_CONTRACT.md)
> **편집 정책:** 동결

이 문서는 웹 UI 이관 전에 사용하던 계약을 내용 그대로 보존한다. 현재 제품의 DOM, 라우팅,
컨트롤러 또는 문구를 판단하는 근거로 사용하지 않는다.

이 문서는 목업(`UI_PROTOTYPE_APPB.html`)의 각 요소가 **어떤 ViewModel 표면**을 렌더/호출하는지의
살아있는 매핑이었다. 목업 요소의 `data-vm="클래스.속성"` 주석과 자동 seam 검사는
Qt 셸 철거와 함께 퇴역했다. 현재 공개 경계는 후속 웹 UI 계약과 제품 브리지 계약이 소유한다.
...
프로파일 IO: `MappingModel.apply_profile`/`to_profile`(작업 저장·편집 복원이 소비하는
링1 seam — 작업이 매핑을 자족 저장·복원한다). (**뒤집힘 2026-07-18, F22**: 링2의 별도
`매핑 프로파일` 저장/적용/삭제 버튼과 공유 베이스 레지스트리는 제거 — 재사용은 홈
「작업 복제」 한 동선으로 수렴. `UX_FINDINGS_101_WALKTHROUGH.md` F22 참조.)
미리보기 위치는 `이전 행 / 행 N/M / 다음 행`.
...
| 층 | 권위 | 워크플로 | 가드 |
|---|---|---|---|
| **구조·seam** (어떤 요소가 어떤 VM 표면에 물리나) | **목업** | 목업에 요소+`data-vm` 먼저 → 계약 테스트 red(VM에 멤버 없음) → 코드가 따라와 green | 당시 자동 seam 검사(퇴역) |
...
**당시 실무 순서(구조 변경 시).** ① 목업에 요소+`data-vm` 추가 → ② 자동 seam 검사의
