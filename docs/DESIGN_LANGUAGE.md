...
- 다크: `accent #6f97e8` / `accent_hover #8badf0` / `accent_tint rgba(111,151,232,.16)` / `accent_ink #9db6f0`
...
1. Segoe/Malgun 스택 → 동봉 Pretendard GOV 이관 시 **exe 크기 비용**(전 웨이트 vs 서브셋) 미확인.
2. WebView2/Chromium 래스터라이저의 Pretendard **소자간 힌팅**이 10~13px 한글 밀집에서 native
   ClearType Malgun과 차이 나는지 미확인(글리프 메트릭은 동일).
