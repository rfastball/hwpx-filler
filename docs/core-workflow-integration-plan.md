...
**슬라이스 2 착수 전 선행 관문(2026-07-26 추가):** v6 상태전이 계약 결함 리뷰 보고서
(사용자 별도 보유 — 세션에 제공 필요)를 triage 한다. 3분류: ①mock(v6.js) 전용 결함 =
폐기 목록(지도 §4)과 함께 소멸 확인 ②계약(core-workflow.md §18·§19) 자체 결함 = 계약
개정 → 지도·계획서 갱신 → 해당 슬라이스 편입 ③기이식 전이(마운트·전환·선택·무효화)
결함 = 즉시 이슈·수선. triage 결과는 지도에 열을 추가하지 말고 별도 절로 박는다.
