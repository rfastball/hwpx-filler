...
- 조치 계획은 [REVIEW_STD_FINDINGS.md](REVIEW_STD_FINDINGS.md) "권장 조치 시퀀스" 6단계가
  원장. 1·2단계(저비용 접근성 + confirm 게이트 수평 전개)가 최우선 — 대부분 국소 수정·기존
  패턴 재사용이라 임팩트/비용 비율 최고.
- 앞선 두 라운드의 조치 방식 계승: 직교 고립단위 · 병렬 워크트리(최신 master 재분기 필수) ·
  적대 검증(수리 전 재현→수리 후 비재현) · 선형 cherry-pick · 스테이지별 통합 게이트.
- UI 캡처 필요 검증(ST-35 등)은 UI 라운드 §5 하네스(offscreen + `QT_QPA_FONTDIR`) 재사용.
