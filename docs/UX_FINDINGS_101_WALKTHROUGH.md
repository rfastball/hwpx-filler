...
> **문서 상태:** 역사 기록
> **권위 범위:** 2026-07-18 워크스루의 관찰·트리아지·처분 근거
> **후속 정본:** 채택 결정은 R-flow·R-info 합의문, 실행 상태는 GitHub 이슈
> **편집 정책:** 동결
...
| **R-flow** | 에디터↔실행 흐름·트랙 파리티 | F18·F20·F21·F19 · F26·F27 · F36·F37·F38·F39 — **합의 종결 2026-07-19 → `docs/R_FLOW_JOB_SESSION.md`**(블록 1~6 + 계약 승계 점검표 부록; F37 재처분·F11 부분 개정 동반) | 5 |
...
| **F11** | 홈 「＋ 새 기안」 (`home.js:314`) | F10과 동형(초기화 없는 bare nav, `txt`도 REFRESH_ON_NAV 밖). 라벨 "새"와 불일치. 단 txt는 일회성이라 소실 위험 없음 *(부분 개정 2026-07-19, R-flow 결정 32: 빠른 기안 수기 폼 신설로 "버릴 durable 상태 없음" 전제 변경 — 새 기안=가드+슬롯 1 덮어쓰기)* | 표면-행동 불일치 | 다듬기 | 코드수정(F10과 묶음) | 기록 |
...
| **F37** | 즉시 기안 저장 | 일괄 저장 부재(복사/저장 건별만). 분할 vs 라인구분 하위결정(기존=라인), **일괄복사는 제외**(건별 붙여넣기) *(재처분 2026-07-19, R-flow 결정 15·18: 일괄 저장 자체가 파생 수요 — 기록 원본=내부 시스템, 파일 산출물 무소유. 처방=전-선언 큐의 루프 마찰 제거, 건별 저장도 동반 사망. 재개 조건=수요 실증)* | 기능 갭(배치 파리티) | 마찰 | R-flow | 기록 |
