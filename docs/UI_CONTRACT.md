...
- **상세 하단은 연결 그림이다**(U6-F #980 · U6 §2.6 · 동결 시안 장면 4). 종전의 사실 3행
  ...
  4열 표(템플릿 필드 · 데이터 열 · 표시형 · 첫 행)와 계획 한 행(파일 이름 + 나머지 건수)이
  「이 작업은 무엇을 무엇으로 채워 어떤 파일을 만드나」를 잇는다(저장 폴더는 전역 설정이라
  여기 재진술되지 않는다 — U6 §5 2026-09-03 재판정 ④). 구획은 셋이고 서는 조건이 갈린다:
...
- 편집기로 가는 손잡이는 **가운데 「연결」 줄 하나**다(`#libraryPairingEdit`, U6 §5
  2026-09-03 재판정 ⑤). 연결선 + 수치(`연결 n / m · 확인 필요 k`, 세지 못한 갈래는 「연결
  확인」)가 버튼이고 편집기 **필드 연결** 단계에 착지한다 — 두 항목 사이의 관계가 곧 편집
  대상이라 그 관계를 그린 줄이 문이다. 종전의 「템플릿 재선택…」·「데이터 재선택…」 두 버튼은
  걷혔다: 데이터 재선택은 「작업 편집」 기본 착지·표 행 클릭과 같은 단계였고(완전 중복), 템플릿
  교체는 매핑까지 가는 정체 교체라 「작업 편집」 → 템플릿 탭이 진다. 손잡이는
  `editWork(name, evidence, {section: "binding"})` → `EditorEntry.openGuarded` 를 타므로 편집기
