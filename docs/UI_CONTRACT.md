...
- **1단계 「고르기」는 두 풀과 한 카드다**(U6-B #976 · U6 §2.2 · 동결 시안 장면 1). 존
  ...
  우 데이터 풀(`#editorDataPool`)을 든다. 전진 동사는 하단 `data-act="next"` 하나이며 중앙 카드는
  짝과 근거를 보여 준다. 소유는 이렇게 갈린다:
  ...
  - **`ready` 는 「짝이 실제로 섰는가」다** — 경로 둘 + **필드 1개 이상**. 경로만 보면 채울
    ...
    카드 옆에서 하단 전진 동사가 비활성으로 남는다(화면이 「짝이 섰다」고 말하면서 다음으로 못 가는 자리).
...
- **2단계 「연결 확인」은 4열 표 하나다**(U6-C #977 · U6 §2.2 · 동결 시안 장면 2). 열은
  ...
  - **행 상태는 닫힌 4태이고 그 라벨은 링1 이 소유한다**: `suggested`(자동 제안) ·
    ...
    현재 활성 데이터의 **원본 키 문자열**이 템플릿 필드명과 정확히 같으면 연결을 자동확정하고
    확인 배지에 「자동확정 · 이름 일치」를 표시한다. 별칭이나 정규화 후 유사도 1.0 은 제안에
    머물며, 정확한 실키가 함께 있으면 그 키를 우선한다. 수동 소스·유형·표시형·상수 변경과
    명시적 확인 해제는 확정을 풀고, 명시적 해제는 재투영·재제안·소스 토글로 되돌아오지 않는다.
    소스가 없어지면 확정을 풀어 기존 경고를 낸다. 저장 프로파일의 사람 확정 복원과 값·서식·
    생성 게이트는 그대로다. TXT 작업대도 같은 판정과 근거 표지를 소비한다.
...
- **TXT 저작 린트메모장**(S10-05 #862 · #299 회수): `#txtEditModal` 의 본문 입력은 textarea 가
  ...
  **판정은 하나도 프런트에 없다** — 타이핑 180ms 디바운스 뒤 `tpl/txt_lint`(`content` 하나,
  ...
  `scan_text_structure`+`scan_text_token_spans` 의 진단·요약은 그대로 싣고, **토큰 문자
  오프셋은 브리지에서 UTF-16 코드 단위로 변환**한다. 표면은 그 좌표에
  `.cm-txtField`/`.cm-txtMarker` 를 얹고 `message` 를 `#txtLintDiag`
...
- 데이터가 없으면 표 검색·정렬·전체 선택·표 자체를 감추고, 선택 안내와 차단 사유는 남긴다.
  실행 전 빈 결과 존은 자리를 차지하지 않으며, 진행 시작부터 결과·거절까지 다시 선다.
...
- **퇴역한 동사**(같은 슬라이스): 폴더 일괄 가져오기(`import_templates_folder` 브리지 ·
  ...
  고르기 열의 「서식 폴더 설정」·「새 TXT 템플릿…」 버튼은 노출하지 않는다.
  루트 변경은 전역 설정, 기존 TXT 편집은 항목의 「내용 편집」이 맡는다.
...
- **상세 하단은 연결 그림이다**(U6-F #980 · U6 §2.6 · 동결 시안 장면 4). 종전의 사실 3행
  ...
  - **표의 행은 편집기와 같은 규칙으로 선다**: 템플릿을 읽었으면 **누름틀 필드**에서 행을
    ...
    주지 않는 것도 계약이다 — 주면 이름이 같은 열에 자동 결속·확정이 붙어 읽기 전용 상세가
    현재 데이터 없이 결론을 지어낸다. 템플릿을 못 읽으면 저장된 연결만 그리고 **그 사실을
...
- 2-pane 공간 배분은 목록 길이에 끌려다니지 않는다: 폭 ≥921px 에서 두 pane 이
  화면의 남은 높이를 나눠 각자 스크롤한다. 극히 낮거나 글자가 큰 창에서는 최소 높이 뒤로
  페이지 스크롤이 열려 행동에 닿는다. 좌:우 비율은 U6-F(#980)에서
