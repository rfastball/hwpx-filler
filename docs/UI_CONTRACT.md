...
- `candidates` 는 4구획이다(슬라이스 2): `top`(상위 `MAIN_TOP_N` available, 링1
  ...
  후보 축에서 미리 말한다(판정 F — 눌러본 뒤 차단은 뒤늦은 경보). **활성 작업이 순위 밖
  available 이면 `top` 말미에 덧붙여 카드를 상시 보장한다**(#342 리뷰 P2 — 존 사망 뒤
  정체·연결 상태·재연결의 화면 내 거처가 활성 카드 하나라, 슬라이스에 잘리면 복구 동선이
  소멸한다). 상위 5 순위는 왜곡하지 않고, 덧붙인 활성은 `more`(잘린 수)에서 뺀다.
