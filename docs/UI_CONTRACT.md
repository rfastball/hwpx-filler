...
`#jobOrderBar` 는 id 를 유지한 채 상시 재진술(`#jobOrderNote`)만 든다.
...
- 순서 변경은 **선택 집합을 바꾸지 않는다**(투영 대 집합). 바뀌는 것은 생성 순서와 그 함수인
  ...
  두지 않는다 — **근거가 U4 8번에서 갈렸다**: 종전 근거는 「표 「문서」 열이 새 이름을 즉시
  보여준다」였는데 그 열이 걷혔다. 지금 근거는 이름이 **그 왕복에 들어갈 가치가 있는 정보가
  아니라는 것**이다(사용자 확정 2026-08-30). 이름을 대조할 자리는 「생성 예정 문서」 존
  (`#jobPlannedDocuments`) 하나이고, 순서가 바꾸는 사실은 위 문안이 상시로 말한다.
...
- **값과 이름은 파생**이다: 값은 실행 입력과 같은 `mapped_records`, 이름은 필터 밖 선택 칩이
  쓰는 그 문자열(`RunStatus.audit`)이다(U4 8번 전에는 표 「문서」 열이 그 소비처였다 — 산출자는
  그대로이고 소비 표면만 바뀌었다). 한 건만 따로 계산하면 `{{seq}}` 가 1 로 고정되고
...
- **승인은 면을 닫고, 거절은 남긴다**(U4 §1-21). 승인은 「이미 확인했다」는 뜻이라 확인하라고
  띄운 면을 그 자리에 세워 둘 이유가 없다 — 더 볼 것이 있으면 다시 여는 것이 그 동사다.
  거절은 반대다: 무엇이 왜 안 됐는지 말할 자리가 그 면이라 닫으면 사유를 잃는다. 닫기는
  `Modal.close` 를 지나므로 `preview_close` 가 함께 나가 열림의 Python 소유가 갈리지 않는다.
  이전 계약(`docs/archive/DATA_FIRST_INTEGRATION_MAP.md:1472` 「승인 뒤 면 생존」)은 이것으로
  **대체됐다** — 그 문서는 동결이라 고치지 않고 여기가 승계 진술이다.
- **이름 충돌은 승인 축이 아니다**(U4 §2-27·2-28). 같은 이름이 있으면 덮어쓰는 것이 기본이고
  (`DEFAULT_COLLISION_POLICY`) 「충돌 처리」 선택기도 「목록 새로 확인」도 화면에 없다. 파괴
  승인을 세우는 것은 정책 의도가 아니라 **이번 계획의 처분**이다: `WRITE_OVERWRITE` 인 항목이
...
- **작업은 데이터를 durable 로 든다**(`Job.data_path`·`data_sheet`·`data_header_row` —
  마운트 시점에 한 벌로 포획한 참조). 이것이 작업↔데이터의 **유일한 관계**다: 스키마
  호환을 후보 축으로 병존시키지 않는다. 쓰는 자리는 **편집기 저장 하나**다.
...
- 데이터·선택·필터는 여전히 **세션 소유**라 vm 재생성에서 생존하고, 잃는 것은 실행
  증거(완주 담보·승인)뿐이다. 구 T1 스위치 가드(`needs_confirm`/`switch_job`)는 파괴가
