...
- **본문 확인 존은 표 없는 한 줄이다**(U2 §2.13 · #346): 빈 값 표지(필드 이름 지목 —
  정보이지 클릭 표적이 아니다) + 이름 건수. 구 거울 테이블(필드 채움 표·클릭형 ack 행·
  420px 캡·펼침 면 `#jobConfirmSheet`)은 필드축 ack 폐기와 함께 사망했고, 그 뒤를 이어
  값·이름을 말하던 확인 면(`#previewSheet`)과 그 출구(「생성 값 미리보기 ⤢」)는 #957
  에서 철거됐다 — **값을 말하는 표면은 만들어진 문서다**. danger 차단 배너(드리프트·
  미해소 토큰)는 같은 자리·같은 형상으로 남는다. 인라인 재진술(`#jobRestate`)은 선택
  유래·산출 수치·경로를 말한다.
