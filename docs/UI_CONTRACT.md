...
- `candidates` 는 4구획이다(슬라이스 2): `top`(상위 `MAIN_TOP_N` available, 링1
  ...
  후보 축에서 미리 말한다(판정 F — 눌러본 뒤 차단은 뒤늦은 경보). **활성 작업 카드는
  호환성과 무관하게 `top` 말미에 덧붙여 상시 보장한다**(#342 리뷰 P2 2건 — 존 사망 뒤
  정체·연결 상태·재연결의 화면 내 거처가 활성 카드 하나라, 슬라이스·호환성에 잘리면 복구
  동선이 소멸한다; 구 존은 호환성과 무관하게 재연결을 제공했다). 상위 5 순위는 왜곡하지
  않고, 덧붙인 **available** 활성은 `more`(잘린 수)에서 뺀다. ranked 밖 활성(needs_action·
  unsupported)의 「연결 상태」는 부재가 있으면 「템플릿 없음」(재연결 리다이렉트 유지),
  아니면 비적격 사유(없는 열 열거·미상 방식)를 정직하게 말한다 — 클릭 리다이렉트는 템플릿
  부재 축만이고 구조 불일치 → 마법사 리다이렉트는 판정 E(#349) 소관이다.
