...
- **상세 하단은 연결 그림이다**(U6-F #980 · U6 §2.6 · 동결 시안 장면 4). 종전의 사실 3행
  ...
  4열 표(템플릿 필드 · 데이터 열 · 표시형 · 첫 행)와 계획 한 줄이 「이 작업은 무엇을 무엇으로
  채워 어떤 파일을 만드나」를 잇는다. 구획은 셋이고 서는 조건이 갈린다:
  - **카드는 언제나 선다.** 정체는 템플릿을 못 읽어도 답할 수 있고, 무엇보다 그 정체를
    바꾸러 가는 동사(재선택)가 거기 있다 — 템플릿이 사라진 갈래에서 카드를 접으면 고치러 갈
    길이 함께 접힌다. 수치(`연결 n / m · 확인 필요 k`)는 셀 수 있을 때만 서고(`card.counted`),
    그 재료는 `domain/fill_ledger.template_path_drift` 가 **매 refresh 한 번** 계산하는
    `TemplateStructureDrift` 다(링1 `JobRow.template_field_count`·`unbound_field_count`·
    `stale_mapping_count`). 저장된 프로파일은 확정 행만 담아 「확인 필요」를 셀 수 없으므로
    그 수치는 현재 템플릿과의 대칭차에만 있다. `stale_count > 0` 이면 「템플릿에 없는 연결
    n건」을 **숨기지 않고** 한 줄 더 적는다. 표시명은 목록·편집기와 같은 규칙이다
    (`library_display_name` / 풀 등록명·basename — `webapp/screens.dataset_display_name`).
  - **표는 템플릿을 읽을 수 있을 때만 선다.** 못 읽는 갈래에서 빈 행 목록으로 서면 수치가
    상태를 참칭한다. 8행을 프레임 안에 두고 나머지는 스크롤로 감추는 대신 **이름으로**
    명시한다(`more_fields`). 행 클릭·Enter 는 `editWork(name, evidence, {target})` 로 편집기
    2단계의 그 행에 착지한다 — `target: binding/<필드>` 는 기존 문맥이고 배관도 이미 서 있다
    (`app.py.open_editor` → `load_job(target=…)` → `aimAtTarget`, 백엔드 신설 0).
  - **계획 한 줄**은 파일을 만드는 갈래에만 선다(TXT 는 `plan`·`output_folder` 둘 다 `null` —
    만들지 않을 파일의 이름과 저장 위치를 말하지 않는다). 이름은 **실제 생성기와 같은 함수**
    (`naming.make_output_filename`)가 만들고, 첫 행을 아직 못 읽었으면 이름 대신 **규칙**
    (`plan.pattern`)을 말한다. 종전의 상세 「파일 이름 규칙」 행이 이 자리로 내려왔다 —
    규칙과 실제 이름을 두 자리에서 말하면 한쪽이 늙는다. 저장 폴더는 작업 화면·편집기와
    **같은 함수**(`webapp/output_folder_zone.output_folder_zone`)가 낸다.
- **「첫 행」은 지연 읽기다**(U6-F #980). 데이터 파일을 열어야 채워지므로 `select_work` 는
  즉시 `first_row.state="pending"` 스냅샷을 내고, 읽기는 **워커 스레드**에서 돈 뒤 **전체
  `library` 스냅샷을 다시 민다**(런타임 `reduce` 는 job 채널만 부분 dict 델타를 허용한다).
  규율 넷은 명시다: ① 결과는 컨트롤러의 **사적 캐시**(키 = 작업 이름 + 참조 4벌)에 들어가
  검색 타이핑의 재렌더가 파일을 다시 읽지 않게 한다(`refresh` 는 캐시를 비우지 않는다 —
  참조가 같으면 값도 같고, 참조가 바뀌면 키가 달라져 자연히 미스다). ② 늦게 끝난 읽기는
  **상관 키를 다시 대조**한 뒤에만 푸시한다(`_push_progress` 의 `run_token` 선례) — 그사이
  다른 행을 골랐으면 캐시에만 남는다. ③ 상태는 명시 3태 `pending`/`ready`/`error` 이고
  `record={}` 로 미읽음을 흉내 내지 않는다(빈 레코드는 미입력 표식을 찍어 「산출물이 담을
  것」과 「아직 모름」을 한 글자로 접는다). ④ 실패는 사유와 함께 그 칸에 서고
  `library_health_causes` 에는 **섞지 않는다**(§19.7 — 호환성·건강과 상세 판정은 분리).
  이 읽기는 「선택 ≠ 착석」 불변을 흔들지 않는다: 편집기·작업 화면의 `load_data_path` 계열을
  재사용하지 않고 자기 캐시에만 남으며 자기 채널만 민다.
...
- 같은 두 항목이 **재선택 바로가기**를 하나씩 더 든다: `#libraryRepickTemplate`(「템플릿
