...
[ADR K 착수] ─▶ KA(조립 코어, 격리 리프) [J1 풀항목 계약]  ─▶ KB(쿼리빌더 GUI)  [J1 필요·데이터셋 풀 표면]
```
...
# ✅ 설계 확정 블록 — ADR L (생성 원장 + 워크플로 드리프트) · **구현 착지(L1·L2 병합, 2026-07-12)**
...
# ✅ 착지 — ADR K (조립 파이프라인) · 2유닛 분할 (KA·KB 병합 완료)
