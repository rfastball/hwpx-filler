...
블록 (ADR L 생성원장 — L-축 미확정, 에이전트 부적합):
  L0(사용자 확정) → L 생성원장+드리프트 감지 (D-8 소스프로파일 해동 동반)
...
[ADR J 확정] ─▶ J1(데이터셋풀) ─▶ J2(매트릭스실행)
                            └──▶ J3
[ADR L 미확정] ─▶ L0(사용자 확정) ─▶ L(생성원장+드리프트)   ← run_state 오라벨 구멍 닫음
...
| `core/mapping.py` | V1, J3 | **V1 먼저**(어휘 탈하드코딩) → J3(오버레이·시드) |
...
# 🚧 설계 미확정 블록 — ADR L (생성 원장 + 워크플로 드리프트)

`UI_DESIGN_DECISIONS.md` ADR L은 **초안**, **L-축 4결정 미확정**:
1. 원장 필드당 스키마(열 집합·직렬화 포맷)
2. 소스 프로파일링 깊이(샘플 N건·타입 라벨 집합) — **D-8 소스 측 해동**(실데이터 확보)
3. 드리프트 감지 트리거 지점(데이터 선택 시·저장 시·주기적?)
4. opt-in 추적 산출 형태(원장 JSON 첨부 vs 별지)

→ **L0 = 사용자 확정**이 선결. 자율 에이전트 부적합(미확정 스키마를 주면 발명 위험).
**단 인접 신호(C-트랙과 맞물림):** ADR L이 **`run_state`의 "의도적 공란 vs 드리프트 유입" 오라벨
구멍을 상태 구별로 닫는다**(ADR J 축2가 지목) — C2 `compile_status`·ADR G 값겹침과 인접 층.
L 착수 시 `core/fill_ledger.py`(신규)·`run_state`·소스 프로파일링 예상. 경계: **값 미리보기 ≠
HWPX 렌더(ADR C 불변)** — 원장은 값만 보이고 문서 렌더 안 함.
