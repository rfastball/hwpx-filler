...
> **웹 이관 후 상태(2026-07-22).** 이 결정의 Qt 링2 서술은 최초 분리 당시의 역사 기록이다.
> 현재 실제 UI는 pywebview + WebView2이며, `webapp`의 브리지/컨트롤러와 `web/` 자산이
> 프레젠테이션 링을 이룬다. `gui` 패키지는 프레임워크 비의존 ViewModel만 보존한다.
...
**현재 실제 UI는 pywebview + WebView2다. 목업은 디자인 계약의 참고 자료다.**

### 링 (의존은 안쪽으로만, 안쪽 링은 UI runtime 미포함)
...
- **링1 — 앱/ViewModel** (UI runtime-free): `mapping_state.MappingModel`·`selection_state.SelectionModel`
  ...
  `template_manager_state`·`nara_state`(전부 GUI runtime 무임포트 — 링0/`data`만 의존,
  2026-07-12 임포트 확인; `matrix_state` 는 F9 로 화면과 함께,
  `vocab_workbench_state` 는 F22 로 공유 베이스와 함께 제거).
  **GUI runtime 임포트 금지.** 변경 통지는 순수 옵저버 콜백. 상태는 직렬화 가능
  dataclass(`JobRow`·`PreflightResult`·`PrevNote`·`GateError`)로 낸다.
- **링2 — 웹 프레젠테이션/브리지 + 토큰** (얇은 어댑터): `webapp/*`와 `web/*`가 ViewModel을
  호출하고 JSON-safe payload로 DOM에 전달한다. 파일 선택·확인·창 수명은 pywebview 브리지에서
  처리한다. **디자인 패스가 만지는 유일한 링**(+ 토큰 파일)이며 링1은 확정된 값만 받는다.

**계약 seam = 링1 ViewModel 공개 API + 상태 dataclass.** ([UI_CONTRACT.md](UI_CONTRACT.md))

### 토큰 파이프라인 (디자인 색 = 데이터, 코드 아님)

`gui/design_tokens.json` 단일 출처 → `scripts/gen_design_tokens.py` 가 실제 웹 CSS와 목업의
`<gen:tokens>` 영역(`--a-*` CSS 변수)을 함께 찍는다. 색 변경 = JSON
1곳 편집 + regen, 백엔드 구조 무접촉. `tests/test_design_tokens.py` 가 드리프트를 CI에서 막는다.
생성물은 **커밋되는 소스**다
...
- 링1은 GUI runtime을 임포트하지 않는다(헤드리스 테스트 가능성이 그 증거).
- 웹 action registry와 bridge/controller 공개 계약은 DOM·selftest 게이트로 보존한다.
