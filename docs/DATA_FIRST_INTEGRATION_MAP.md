...
| F6 | TXT 합류 + 작업대 → `draft` 흡수·사망(휘발 세션 폐지 고지) **+ F5 드로어 행별 「수정」 deep-link 동승**(§10.14.3) — **PR 2분할**: A(TXT 합류·작업대 신설, `draft` 존치) 계약 §10.15 / B(`draft` 사망·승계 정산·deep-link) | 17·18·19 |
...
#### 10.14.3 남는 것 하나 — F5 드로어 행별 「수정」 deep-link
...
### 10.15 F6 PR-A 착수 계약 — TXT 합류 + 검토·복사 작업대 (2026-07-28)
...
| 2 | **어휘 통일** — 링0 `work_mode` · 라벨 단일 출처 · `compatibility_for` 매체 국경 해제 · 고정 테스트 2건 뒤집기(둘 다 "F6 이 오면 시끄럽게 알린다" 주석 보유) · 회귀 |
| 3 | **후보·탐색 방식 구획** — Top 5 결과의 방식 구획(1방식 평면 퇴화) · 카드 부제 방식 텍스트 · 매체별 최근 사용 문안 · 탐색 시트 탭 안 구획 · 실행 버튼 2분기 · DOM 계약 · selftest 프로브 |
| 4 | **작업대 표면** — `scr-workbench` · 고정 사본 세션 · 좌 필드 연결(`EditSession` section=binding) · 우 원문/채운 모습 + 검토 상태 · footer(작업점 이동·자동 다음·복사) · 「기본 규칙으로 저장…」 · 이탈 가드 · action registry · UI_CONTRACT · selftest 프로브 |
