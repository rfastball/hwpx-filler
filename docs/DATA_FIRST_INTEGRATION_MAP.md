...
| ~~F4~~ **착지** | 결과 3태 + 부분 실패 표면 — **재시도 3종은 F7 로**(`runOverrides` 선행) — PR #311 squash `1ea532e` · 계약 §10.10 · 정산 §10.10.5 · 리뷰 4라운드 §10.10.6·8·9 | 8·9 |
| ~~F5~~ **착지** | 미리보기 드로어 + 검토 요구(`ReviewRequirement`) — **구조 검토는 F8·override 는 F7** — PR #313 squash `8dcc209` · 계약 §10.12 · 정산 §10.12.4 · 리뷰 5라운드 §10.12.5~§10.12.10 | 16 |
| F6 | TXT 합류 + 작업대 → `draft` 흡수·사망(휘발 세션 폐지 고지) | 17·18·19 |
| F7 | 편집기 탭 재편 + `EditContext`·patch 거래 + 판본 + `runOverrides` — **PR 2분할**(사용자 확정): A(몰입 표면·탭·patch·판본) 계약 §10.13 / B(`runOverrides` + F4 재시도 3종 + F5 deep-link·배지) | 20·21·22·23 |
...
### 10.13 F7 PR-A 착수 계약 — 편집기 몰입 표면·탭 + patch 거래 + 판본 (2026-07-27)
