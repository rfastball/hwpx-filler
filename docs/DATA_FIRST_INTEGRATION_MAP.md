...
| F2 | 전역 라이브러리 표면(browser+detail) + 상단 2탭 → `home` 흡수·사망 — 계약 §10.8, **PR 2분할**(A=표면·`home` 사망 / B=셸 교체·좌 목록 사망) | 14·15·1 |
...
**H. `문서 만들기에서 사용` 3분기는 링2가 가르고, preferredWorkId 는 이 슬라이스가 채운다.**
§19.8 의 세 갈래는 현재 데이터의 준비·호환 여부를 요구하는데 그 상태는 「문서 만들기」
스냅샷이 이미 링2에 있다 — 홈이 카드에서 대상 화면을 미리 겨누던 선례와 동형이라 **새 백엔드
결합 없이** 웹이 가른다.

```text
데이터 ready + 호환   → select_job 겨눔 → 문서 만들기 복귀(RecordRangeState 유지)
데이터 ready + 비호환 → 문서 만들기 복귀 + 문서 탐색 「확인 필요」 탭에서 그 작업으로 focus
데이터 없음           → preferredWorkId 보관 → 문서 만들기 복귀(데이터 선택 뒤 호환성 판정)
```
