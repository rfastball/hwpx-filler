...
| F5 | 미리보기 드로어 + 검토 요구(승인) — **구조 검토는 F8·override 는 F7** — 계약 §10.12 | 16 |
...
### 10.12 F5 착수 계약 — 미리보기 드로어 + 검토 요구(승인) (2026-07-27)
...
| `semantic_binding` | 바뀐 필드의 현재 값 + 선택분 중 **서로 다른 값의 수** + **새로 비는 레코드 수** | `mapped_records` 집계(거울의 `_field_value_display` 와 같은 술어) |
...
생긴다. 그래서 `template` 축 변경은 `riskClass` 서열에 남기되(무엇이 바뀌었는지는 말한다)
승인 사건을 요구하지 않는다. 구조 검토 표면은 계약대로 **F8**(템플릿 바꾸기·시험 탭) 소관.
