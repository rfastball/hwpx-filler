...
3. **기존 characterization과의 계약 충돌 2건** — 초기 선택(전체→0건), 표시·실행 순서
   (원본 오름차순→최신 행 먼저 + 표시순 투영). 둘 다 v6 계약이 정본이므로 채택하되,
   기존 테스트 개정이 커밋 1의 명시 작업이다(§2 상세).
...
= 실행 입력 순서가 표시 순서를 따른다. **v6 채택**(계약 정본). ⚠ 파장이 사용자 가시적이다:
같은 선택이라도 **생성 순서(파일 생성·결과 나열 순)가 기존과 달라진다**. 커밋 1에서 이
전이를 characterization으로 명시 고정하고, 필요 시 정렬 토글은 후속.
