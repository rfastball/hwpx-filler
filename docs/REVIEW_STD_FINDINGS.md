...
>
> **조치 완료(2026-07-13): 착지 21 · 보류 1(ST-14) · 재분류 1(ST-33).**
> 유닛 U1~U8 · 6스테이지 직렬 착지(각 스테이지 ruff·pyright·pytest 통과, 최종 821 passed).
> 착지 기록은 아래 "스테이지 착지 기록" 절.
> **ST-01 셸 리팩터는 별도 전용 라운드(2026-07-13, S1~S7)로 착지** — 설계서
> [SHELL_DESIGN.md](SHELL_DESIGN.md), 단일창 네비 레일 셸 + 위저드 모달, 최종 834 passed.
...
**보류·재분류(2) + 별도 라운드 착지(1):**
- **ST-01**(중간) — 다중창→단일창 셸 리팩터는 **별도 전용 라운드**(설계부터)로 분리했고,
  **같은 날 착지 완료**: 설계서 [SHELL_DESIGN.md](SHELL_DESIGN.md), 스테이지 S1~S7
  (설계 → 셸 골격+can_leave 추출 → 홈 임베드 → 관리 4종 임베드 → run/txt 임베드+동적
  레일 → 위저드 ApplicationModal+싱글턴 해체 → 잔재 정리), 게이트 834 passed. 이
  라운드가 창 수명(U3)·재사용(U5)을 먼저 마감해 리팩터 표면을 줄여 둔 덕이다.
