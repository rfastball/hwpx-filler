...
| [온보딩 튜토리얼](ONBOARDING_TUTORIAL.md) | 유효 결정 | 배포본 동봉 예제 세트(#284 승계)와 진행 감지 체크리스트의 설계 — 자산 명세·루프 커리큘럼(기본/응용/고급 티어, 기존 UX 루프 문서 역참조)·단계 판정·설치/제거 경로. #284 결정 ②(범용 중립)를 조달 실문서 각색으로 뒤집는 재판정 포함. 구현 추적은 슬라이스 이슈 |
