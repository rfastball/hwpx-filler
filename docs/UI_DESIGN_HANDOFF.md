...
| 심볼 | 계약 |
|---|---|
| `Job(name, template_path, mapping: MappingProfile, filename_pattern, version)` | durable 바인딩 `{템플릿·매핑·파일명}`. **데이터·행은 없음**(실행 일회성). `template_fields()`·`source_keys()`·`to_dict/from_dict/save/load`. |
