...
> **유래.** 이 문서는 랩 브랜치 `lab/ui-reboot`(태그 `prototype-v6-freeze`, 커밋 `2498604`)에서
> 태어나 그동안 그쪽에만 있었다. 봉합 지도가 계약 정본으로 열네 곳에서 참조하는 문서가
> 병합되지 않은 브랜치에만 사는 상태를 끝내려고 본문 그대로 옮겼다 — 이 두 블록만 덧붙였고
> 아래 §1 부터는 한 글자도 고치지 않았다.
>
> 본문이 언급하는 `core-workflow-ui-mvp-demo-v6.html`·`core-workflow-prototype/v6.css` 는
> **여기 없다** — 목업·프로토타입 자산은 랩에 남아 있다(`git show prototype-v6-freeze:docs/…`).
> 목업과 이 계약이 어긋나면 **계약이 이긴다**. 목업은 계약의 한 렌더일 뿐이다.
