...
> **문서 상태:** 부분 대체
> **권위 범위:** v4~v6 워크플로의 상태·전이·불변식 계약
> **후속 정본:** 이미 확정된 개정분은 [data-first 봉합 지도](archive/DATA_FIRST_INTEGRATION_MAP.md)
> §7 triage·§8.2 가 이 문서 원문보다 **우선**한다(특히 §18.3 자동선택→추천, §19.4 완주 스탬프 유지)
> **편집 정책:** 동결 — 아래 유래 시점의 원문을 그대로 유지한다

> **유래.** 이 문서는 v6 UI 랩(태그 `prototype-v6-freeze`, 커밋 `2498604`)에서 태어나 그동안
> 그쪽에만 있었다. 봉합 지도가 계약 정본으로 열네 곳에서 참조하는 문서가 병합되지 않은
> 브랜치에만 사는 상태를 끝내려고 본문 그대로 옮겼다 — 이 두 블록만 덧붙였고 아래 §1 부터는
> 한 글자도 고치지 않았다. 랩 브랜치 `lab/ui-reboot` 는 그 뒤 은퇴했고, 최종 상태는 태그
> `archive/lab-ui-reboot` 가 보존한다.
>
> 본문이 언급하는 `core-workflow-ui-mvp-demo-v6.html`·`core-workflow-prototype/v6.css` 는
> **여기 없다** — 목업·프로토타입 자산은 그 아카이브 태그에 남아 있다
> (`git show archive/lab-ui-reboot:docs/…`). 목업과 이 계약이 어긋나면 **계약이 이긴다**.
> 목업은 계약의 한 렌더일 뿐이다.
...
라이트/다크의 단일 출처는 `src/hwpxfiller/viewmodel/design_tokens.json`이며, 토큰이 바뀌면 시안도
같이 바뀐다.
