...
/* 이스케이퍼는 단일 출처 esc.js 것만 쓴다(로컬 재정의 금지 — 게이트 K1). 전역 조회 대신
   정적 import 라 미로드 시점 참조가 런타임 undefined 가 아니라 해석 실패로 드러난다.
   `esc` 는 호출부 가독성용 별칭일 뿐이고, 산출 마크업은 종전과 바이트 동일하다. */
const esc = escHtml;
...
/* 공개면은 예나 지금이나 이 둘뿐 — 전역 대신 이름있는 export 로만 나간다. */
export const SegView = { paint, plain };
