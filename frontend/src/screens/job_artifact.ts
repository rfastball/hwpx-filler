/* 산출물 관찰 시트 — 만들어진 문서의 실물을 읽기 전용으로 보는 면(S7-03 · #825).
   ...
   ## 미리보기와 다른 표면이다(#820 D4)

   `job_preview.ts` 는 **생성 전** 의 예고를 그린다. 이 파일이 그리는 것은 **생성 후** 실물,
   그것도 디스크에서 다시 읽어 기록 digest 와 대조를 통과한 bytes 의 구조다. 둘이 같은 낱말과
   같은 시각 어휘를 쓰면 사용자는 지금 보고 있는 것이 예고인지 실물인지 구분하지 못한다.
   그래서 골격 관용구(`.modal-card .sheet-card`·`.sheet-head`/`.sheet-body`)만 공유하고
   클래스는 `artifact-*`, 제목과 문안은 '산출물 관찰' 계열로 간다.
