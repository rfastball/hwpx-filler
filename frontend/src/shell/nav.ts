 ...
 * 무동작 이행 약속은 집행자가 돌려준다(#490 예약 전역 그물이 app.js 접촉을 기등재한
 * 이유이기도 하다 — 판독을 옮기면 그물이 넓어진다). 음성 게이트(shell_nav.test.js)는
 * 주석까지 읽는다 — 전역 이름을 여기 표기하지 않는 것 자체가 계약이다(n06 규율).
 *
 * ## 수명·구성
 *
...
/** 집행자 — 셸 판정 하나의 DOM·브리지 효과 전부. 판정은 기계가, 집행은 이쪽(app.js
