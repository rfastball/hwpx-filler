...
def main() -> int:
    ...
    for name in ("hwpx_filler.spec", "hwpx_diff.spec"):
        assert "SLIM_QT_BINARIES" in specs[name], f"{name}: Qt DLL 훅 필터 누락"
        assert 'startswith("qt6qml")' in specs[name], f"{name}: QML DLL 필터 누락"
